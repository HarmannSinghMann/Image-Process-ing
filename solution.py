...
if __name__ == "__main__":
    ...
    #Brightness
    factor_b = 2
    image_b  = brightness(image, factor_b)
    cv2.imshow("Brightness-Image Factor:"+str(factor_b), image_b)

    
    
    # Blur
    sigma = 1
    image_blurr = Blur(image, sigma)
    cv2.imshow("Blurr-Image Sigma:"+str(sigma), image_blurr)
    cv2.imwrite('blur.jpg',image_blurr)
    
    
    #Edge Detection - On Grayscale Image
    edge_kernel = np.array([[-1, -1, -1], [-1, 8, -1], [-1, -1, -1]])
    image_edge = convolution(img_gray, edge_kernel)    #Here i will use grey image instead of rgb because of fixed edge_kernel of size 3*3 . 
    cv2.imshow("EdgeDetection-Image", image_edge)
    
    
    #Sharpen - On Grayscale Image
    sharp_kernel = np.array([[-1,-1,-1], [-1,9,-1], [-1,-1,-1]])
    image_sharpen = convolution(img_gray, kernel=sharp_kernel)
    cv2.imshow("Sharpen-Image", image_sharpen)
    
    #Contrast
    factor_c = 1
    image_contrast = contrast(image,factor_c)
    cv2.imshow("Contrast-Image Factor:"+str(factor_c), image_contrast)
